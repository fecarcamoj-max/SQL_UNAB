...
create database diplomado_unab_s2;

# Eliminar una base de datos.
DROP DATABASE diplomado_unab_s2;

# Mostrar bases de datos en el servidor:
SHOW DATABASES;

# Seleccionar la base de datos.
USE diplomado_unab_s2;
...
SHOW TABLES FROM diplomado_unab_s2;#
...
################
SELECT * FROM persona;
### Mostrar cuantos nacimientos hubieron por año. WITH ROLLUP. 
SELECT YEAR(fecha_nacimiento), apellido, count(*) FROM persona
GROUP BY YEAR(fecha_nacimiento), apellido; # YEAR, MONTH, WEEK
### HAVING 
SELECT YEAR(fecha_nacimiento), apellido, count(*) FROM persona
GROUP BY YEAR(fecha_nacimiento), apellido
HAVING COUNT(*) > 1; # REEMPLAZO DEL WHERE DESPUÉS DEL GROUP BY

# WITH ROLLUP:::
SELECT YEAR(fecha_nacimiento), count(*) FROM persona
